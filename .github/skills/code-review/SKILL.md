---
name: code-review
description: "Review pull requests and diffs in terra — the Go wrapper around Terraform and Terragrunt — against the rios0rios0/guide standards, with extra weight on destructive-command safety, parallel execution, and this repository's build-tag policy. Use when reviewing a PR, a branch, or staged changes here."
---

# Code review — `terra`

`terra` wraps Terraform and Terragrunt with path-based infrastructure management inspired by Kubernetes — `terra apply --all /path`, parallel module execution, cache clearing, and toolchain installation. It runs `apply` against real infrastructure, so anything that can skip a confirmation or widen a blast radius is Critical.
...
## What matters most in `terra`

These are the checks that catch real defects in this repository. Work through
them before the generic ones.

- **Auto-approval must stay explicit and narrow.** `--yes`/`-y` means non-interactive *and* auto-approve; `--no` means non-interactive only, so Terraform's prompt aborts. A change that makes auto-approve the default, or that lets `--no` proceed, is a Critical finding.
- **Module selection flags must not silently widen scope.** `--only`, `--skip`, `--filter`, and `--queue-exclude-dir` narrow what runs. A refactor that turns an empty selection into "everything" would apply the whole tree — check the empty and malformed cases explicitly.
- **Parallel execution needs a bound and a failure policy.** `--parallel=N` defaults to 5. A failure in one module must not leave the others half-applied without a clear report, and output from concurrent modules must stay attributable.
...
- One test function per public method, with `t.Run()` cases inside it.
- `testify` throughout: `assert.*` for non-critical checks, `require.*` for critical preconditions.
- A change to flag parsing needs a case for the empty value and the malformed value, not only the happy path.
