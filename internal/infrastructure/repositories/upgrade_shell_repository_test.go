...
func TestExtractQueueScopingFlags(t *testing.T) {
	...
	t.Run("should propagate --queue-exclude-dir with its space-form value", func(t *testing.T) {
		t.Parallel()
		// given
		arguments := []string{"apply", "--all", "--queue-exclude-dir", "excluded-mod", "-auto-approve"}

		// when
		got := repositories.ExtractQueueScopingFlagsPublic(arguments)

		// then
		assert.Equal(t, []string{"--all", "--queue-exclude-dir", "excluded-mod"}, got)
	})

	t.Run("should propagate --queue-exclude-dir in --flag=value form", func(t *testing.T) {
		t.Parallel()
		// given
		arguments := []string{"apply", "--all", "--queue-exclude-dir=excluded-mod"}
	...
	t.Run("should propagate --filter in both space and equals forms", func(t *testing.T) {
		t.Parallel()
		// given
		spaceForm := []string{"plan", "--all", "--filter", "!excluded-mod"}
		equalsForm := []string{"plan", "--all", "--filter=!excluded-mod"}
		...
		// then
		assert.Equal(t, []string{"--all", "--filter", "!excluded-mod"}, gotSpace)
		assert.Equal(t, []string{"--all", "--filter=!excluded-mod"}, gotEquals)
	})
