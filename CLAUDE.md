...
- **CAS:** Terragrunt `1.1+` ships CAS (Content Addressable Store) as a stable, default-on feature, so terra no longer sets the completed `TG_EXPERIMENT=cas` (which now emits an "experiment already completed" warning). `TERRA_NO_CAS=true` opts out by setting the stable `TG_NO_CAS=true` flag; the default leaves CAS on and clears any stray inherited `TG_NO_CAS`
...
- **Parallel output prefixing:** On the terra-managed `--parallel=N` path, each worker's Terragrunt stdout/stderr is streamed through a `LinePrefixWriter` and tagged with the module's base directory name (e.g. `[module-a]`), colorized per module on a TTY (honoring `NO_COLOR`) and serialized through a shared mutex so lines never interleave mid-line. Wired through the `ParallelShellRepository` port (implemented by `StdShellRepository`); the writer lives in `internal/infrastructure/repositories/line_prefix_writer.go`. `ParallelStateCommand` derives the label with `filepath.Base(modulePath)` and calls `ExecuteCommandWithPrefix`. Stdin is disconnected on this path (parallel requires `--yes`/`--no`)
